...
newer paper version: https://aclanthology.org/2021.naacl-main.323.pdf
github: https://github.com/rpryzant/causal-bert-pytorch
"""
...
class CausalBert(DistilBertPreTrainedModel):
    """The model itself."""
    def __init__(self, config,confounds=True):
        super().__init__(config)

        self.num_labels = config.num_labels
        self.c_num_labels = 2 if confounds else 0
        ...
        for T in range(2):
            ...
            self.Q_cls['%d' % T] = nn.Sequential(
                nn.Linear(config.hidden_size + self.c_num_labels, 200),
                nn.ReLU(),
                nn.Linear(200, self.num_labels))

        self.g_cls = nn.Linear(config.hidden_size + self.c_num_labels, 
    ...
    def forward(self, W_ids, W_len, W_mask, C, T, Y=None, use_mlm=True):
        ...
        if C is not None:
            C_bow = make_bow_vector(C.unsqueeze(1), self.c_num_labels)
...
class CausalBertWrapper:
    ...
    def train(self, texts, treatments, outcomes, confounds=None, 
            learning_rate=2e-5, epochs=3):
        dataloader = self.build_dataloader(
            texts, treatments, outcomes, confounds=confounds)
    ...
    def inference(self, texts, confounds=None, outcome=None):
        self.model.eval()
        dataloader = self.build_dataloader(texts, confounds=confounds, outcomes=outcome,
            sampler='sequential')
        Q0s = []
        Q1s = []
        Ys = []
    ...
    def ATE(self, W, Y=None, C=None, platt_scaling=False):
        Q_probs, _, Ys = self.inference(W, confounds=C, outcome=Y)
    ...
    def build_dataloader(self, texts, treatments=None, outcomes=None, confounds=None, 
        tokenizer=None, max_length=50, sampler='random'):
        ...
        if treatments is None:
            treatments = [-1 for _ in range(len(texts))]
        if outcomes is None:
            outcomes = [-1 for _ in range(len(texts))]
        if confounds is None:
            confounds = [-1 for _ in range(len(texts))]
...
if __name__ == '__main__':
    ...
    # load datasets
    df_train = pd.read_csv(args.train_path,lineterminator='\n')
    df_test = pd.read_csv(args.test_path,lineterminator='\n')
    df_val = df_train.sample(frac=0.1,random_state=33)
    df_train = df_train.drop(df_val.index)

    # model
    cb = CausalBertWrapper(batch_size=args.batch_size,
        g_weight=0.1, Q_weight=0.1, mlm_weight=1, confounds=False)

    # train
    cb.train(df_train['text'], df_train['T'], df_train['label'], 
             epochs=args.num_epoch, learning_rate=args.lr) #  confounds=df['C']

    # inference
    probs, preds, Ys = cb.inference(df_test['text'],outcome=df_test['label']) # , confounds=df['C']
    ...
    print(cb.ATE(df_test.text, platt_scaling=True)) # C=df['C'], 

    # cb.train(df['text'], df['T'], df['Y'], confounds=df['C'], epochs=1)
    # probs, preds, Ys = cb.inference(df['text'], confounds=df['C'])
    # print(probs)
    # print(preds)
    # print(Ys)
    # print(cb.ATE(df.text, C=df['C'], platt_scaling=True))
